def find_language_information(languages, _language_name)
  ...
  languages[_language_name]
end

def add_information_about_language(languages, _language_name, _info_key, info_value)
  ...
  languages[_language_name][_info_key] = info_value
...
def find_beautiful_languages(languages)
  ...
  languages.select { |language, data| language if data[:is_beautiful?] }
end
...
def find_language_facts(languages, language_name, _fact_index = 0)
  ...
  languages[language_name][:facts][_fact_index] if languages[language_name]
end
